...
import { AGENT_HEADING, analyze, hasMainHeading, hasNodeOfType, headingKey, INSTALL_HEADINGS, LICENSE_HEADING, parse, RECOMMENDED_HEADINGS, SERVICE_MARKERS, USAGE_HEADINGS, walk, } from './readme-ast.js';
const validatePositioning = (positioning, leadNodes, errors) => {
    if (!positioning) {
 ...
 */
const validateAgentBlock = (agent, errors, warnings) => {
    if (agent.headingCount > 1) {
        errors.push(`Found ${agent.headingCount} "## ${AGENT_HEADING}" sections; there must be one.`);
    }
    ...
    for (const heading of sectionHeadings) {
        if (recommendedIdx(heading) === -1) {
            warnings.push(`Non-standard section "### ${heading}"; prefer ${RECOMMENDED_HEADINGS.map(h => `"${h}"`).join(', ')}.`);
        }
    }
    if (known.length === 0) {
        warnings.push('The block has only positioning; consider adding "When to use" and/or "Common pitfalls".');
    }
    const orderIdx = known.map(recommendedIdx);
    if (orderIdx.some((v, i) => i > 0 && v < orderIdx[i - 1])) {
        warnings.push(`Sections are out of the recommended order (${RECOMMENDED_HEADINGS.join(' → ')}).`);
    }
    // No badges / images inside the block.
    let hasBadge = hasNodeOfType(blockNodes, ['image', 'imageReference']);
    const svcHits = new Set();
    const scanHtml = (n) => {
        if (n.type !== 'html')
            return;
...
export const validatePackageReadme = (content) => {
    const errors = [];
    const warnings = [];
    ...
    if (!hasMainHeading(main, [LICENSE_HEADING])) {
        errors.push(`Missing "## ${LICENSE_HEADING}" section. Example:\n\n` +
            `## ${LICENSE_HEADING}\n\n` +
 ...
 */
export const validateComponentReadme = (content) => {
    const errors = [];
    const warnings = [];
