...
__export(src_exports, {
  ...
  validateComponentReadme: () => validateComponentReadme,
  validatePackageReadme: () => validatePackageReadme
});
...
function base() {
  ...
  function runSync(node2, file) {
    let result;
    let complete;
    processor2.run(node2, file, done);
...
var parse2 = (content3) => processor.parse(content3);
...
var sliceNodes = (content3, nodes) => {
  ...
  if (start === void 0 || end === void 0)
    return "";
  return content3.slice(start, end);
};
var walk = (node2, fn) => {
  fn(node2);
...
var analyze = (content3) => {
  const main = parse2(content3).children;
  ...
  return {
    ...
    agent: {
      ...
      blockNodes,
      leadNodes,
      block: sliceNodes(content3, blockNodes),
      positioning: sliceNodes(content3, leadNodes).trim() || null,
      prose: sliceNodes(content3, proseNodes).trim() || null
    }
  };
};
var sectionBody = (main, content3, names, depths = [2]) => {
  for (const name of names) {
    ...
    return sliceNodes(content3, bodyNodes).trim() || null;
  }
  return null;
};
var dropSubsections = (nodes, exclude) => {
  ...
  for (const node2 of nodes) {
    if (skipAboveDepth !== null) {
      if (node2.type === "heading" && node2.depth <= skipAboveDepth) {
        skipAboveDepth = null;
      } else {
        continue;
      }
    }
    if (node2.type === "heading" && matchesHeading(toString(node2), exclude)) {
      skipAboveDepth = node2.depth;
      continue;
    }
    kept.push(node2);
  }
...
var mergedSectionBodies = (main, content3, names, depths = [2], exclude = []) => {
  const parts = [];
  main.forEach((node2, idx) => {
    if (node2.type !== "heading" || !depths.includes(node2.depth) || !matchesHeading(toString(node2), names)) {
      return;
    }
    const after = main.slice(idx + 1);
    const nextSection = after.findIndex((n) => n.type === "heading" && n.depth <= node2.depth);
    const bodyNodes = nextSection === -1 ? after : after.slice(0, nextSection);
    const kept = exclude.length ? dropSubsections(bodyNodes, exclude) : bodyNodes;
    for (const n of kept) {
      const body = sliceNodes(content3, [n]).trim();
      if (body)
        parts.push(body);
    }
  });
...
// src/readme-parse.ts
var extractMeta = (content3) => {
  const { agent } = analyze(content3);
  if (!agent) {
    return { found: false, block: null, positioning: null };
  }
  return { found: true, block: agent.block, positioning: agent.positioning };
};
var parsePackageReadme = (content3) => {
  const { main, agent } = analyze(content3);
  return {
    agentPositioning: agent?.positioning ?? null,
    agentProse: agent?.prose ?? null,
    install: sectionBody(main, content3, INSTALL_HEADINGS, [2, 3]),
    usage: mergedSectionBodies(main, content3, USAGE_HEADINGS, [2], INSTALL_HEADINGS)
  };
};
var parseComponentReadme = (content3) => {
  const main = parse2(content3).children;
...
var validatePackageReadme = (content3) => {
  const errors = [];
  const warnings = [];
  const { main, agent } = analyze(content3);
...
var validateComponentReadme = (content3) => {
  const errors = [];
  const warnings = [];
  const main = parse2(content3).children;
